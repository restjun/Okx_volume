...
def calc_rsi_3(df, period=3):
    return calc_rsi(df, period)

def calc_mfi_3(df, period=3):
    return calc_mfi(df, period)
...
# =========================
# 4H RSI/MFI 돌파 확인 (3일선/5일선 선택 가능)
# =========================

def check_4h_mfi_rsi_cross_custom(inst_id, period=5, threshold=30):
    df = get_ohlcv_okx(inst_id, bar='4H', limit=200)
    if df is None or len(df) < period + 1:
        return False, None

    if period == 3:
        mfi = calc_mfi_3(df, period)
        rsi = calc_rsi_3(df, period)
    else:
...
def send_new_entry_message(all_ids, period=5):
    ...
    # === 당일 신규 돌파 확인 ===
    for inst_id in top_ids:
        is_cross_4h, cross_time = check_4h_mfi_rsi_cross_custom(inst_id, period=period, threshold=30)
    ...
    message_lines = [f"🆕 당일 신규 돌파 코인 👀 ({period}일선 기준, RSI/MFI 30 돌파)"]
...
def main():
    ...
    # 3일선 신호
    send_new_entry_message(all_ids, period=3)
    # 5일선 신호
    send_new_entry_message(all_ids, period=5)
