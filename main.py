...
def send_new_entry_message(all_ids):
    ...
    sorted_by_volume = sorted(volume_map, key=volume_map.get, reverse=True)[:200]  # 상위 100개만 처리
    ...
    # 상승률 기준 내림차순 정렬 후 상위 3개만 표시
    new_entry_coins.sort(key=lambda x: x[1], reverse=True)  # x[1] = daily_change
    top3_coins = new_entry_coins[:3]

    message_lines = ["🆕 당일 신규 돌파 코인 👀 (4시간봉 기준)"]
    for idx, (inst_id, daily_change, volume_24h, coin_rank, cross_time) in enumerate(top3_coins, start=1):
        ...
        message_lines.append(
            f"📈 상승률 {idx}위: {name}\n"
            f"🟢🔥 {daily_change:.2f}% | 💰 거래대금 Rank {volume_rank} ({volume_str}M)\n"
