...
# ===== USDT 도미넌스 계산 (간단 예시, 실제 OKX에는 정확한 시총 API 없음) =====
def calculate_usdt_dominance():
    # 여기서는 USDT 거래량 비율 예시로 처리
    ...
    try:
        ...
        usdt_volume = 0
        for item in data:
            vol = float(item["volCcyQuote"])
            total_volume += vol
            if "USDT" in item["instId"]:
                usdt_volume += vol
        dominance = (usdt_volume / total_volume) * 100 if total_volume > 0 else None
...
def send_top_volume_message(top_ids, volume_map):
    ...
    usdt_dom = calculate_usdt_dominance()
    usdt_dom_str = f"USDT Dominance: {usdt_dom}%" if usdt_dom else "USDT Dominance: N/A"
